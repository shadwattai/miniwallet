...
const toggleUserStatus = async (user: User, event: Event) => {
    ...
    const confirmMessage =
        user.status === 'active' ? `Are you sure you want to deactivate ${user.name}?` : `Are you sure you want to activate ${user.name}?`;
...
<template>
    ...
    <div style="padding-top: -10px">
        ...
        <div class="py-12 text-center text-gray-500" v-if="!props.users || props.users.length === 0">
            <i class="pi pi-users mb-4 text-6xl text-gray-300"></i>
            <h3 class="mb-2 text-xl">No Users Found</h3>
        ...
        <div class="grid grid-cols-5 gap-4" style="margin-top: 12px; margin: 12px" v-if="props.users && props.users.length > 0">
            ...
            >
                <template #header>
                    <img :src="getAvatarUrl(usr)" :alt="usr.name" class="h-64 w-full object-cover" />
                </template>
                <template #title>
                ...
                <template #footer>
                    <div class="mt-auto flex gap-4">
                        <Button
                            label="PROFILE"
                            :severity="usr.status === 'active' ? 'success' : 'secondary'"
                            outlined
                            variant="outlined"
                            class="w-full"
                            @click="openUserProfile(usr)"
                            size="small"
                        />
                    </div>
                </template>
            ...
            <div v-if="filteredUsers.length === 0 && filters.global.value" class="col-span-5 py-8 text-center text-gray-500">
        ...
        <div v-if="selectedUser" class="space-y-4">
            <div class="text-center">
                <img
                    ...
                    class="mx-auto mb-4 h-24 w-24 rounded-full border-4 border-gray-200 object-cover"
                />
                            ...
                            <div class="border-b pb-4">
                                <div class="space-y-2">
                                    <div class="flex items-center" style="padding-bottom: 10px">
                                        <i class="pi pi-id-card mr-3 text-gray-500"></i>
                                        <div>
                            ...
                            <div class="border-b pb-4">
                                <div class="space-y-2">
                                    <div class="flex items-center" style="padding-bottom: 10px; padding-top: 10px">
                                        <i class="pi pi-sparkles mr-3 text-gray-500"></i>
                                        <div>
    ...
    <Dialog
        ...
        :modal="true"
        :closable="false"
        class="mx-auto max-w-lg p-4"
        :style="{ width: '90vw', maxWidth: '35rem' }"
    >
        <div>
            <!-- Registration form fields -->
            <div class="field mb-4">
                <label for="reg-name" class="mb-2 block text-sm font-medium">Name</label>
                <InputText
                    id="reg-name"
            ...
            <div class="field mb-4">
                <label for="reg-email" class="mb-2 block text-sm font-medium">Email</label>
                <InputText
                    id="reg-email"
                    v-model="newUserForm.email"
                    :class="{ 'p-invalid': formErrors.email }"
                    placeholder="Enter accessible email"
                    class="w-full"
                    autocomplete="off"
                />
            ...
            <div class="field mb-4">
                <label for="reg-phone" class="mb-2 block text-sm font-medium">Phone</label>
                <InputGroup>
                    ...
                    <InputMask
                        ...
                        placeholder="Accessible phone number"
                        class="w-full"
                        mask="(999) 999-999"
                        autocomplete="off"
                    />
            ...
            <div class="flex justify-end gap-2">
                ...
                <Button label="Register" icon="pi pi-check" severity="success" :loading="isSubmitting" @click="submitRegistration" />
            </div>
        </div>
    </Dialog>
</template>
