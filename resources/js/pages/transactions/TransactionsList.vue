...
const transactionStats = computed(() => {
    ...
    const thisMonthTransactions = transactions.filter((t) => new Date(t.created_at) >= thisMonth && t.status === 'completed');

    const totalIncome = thisMonthTransactions.filter((t) => t.type === 'deposit').reduce((sum, t) => sum + (Number(t.amount) || 0), 0);

    const totalExpense = thisMonthTransactions
        .filter((t) => t.type === 'withdrawal' || t.type === 'transfer')
        .reduce((sum, t) => sum + (Number(t.amount) || 0) + (Number(t.commission_fee) || 0), 0);

    const totalFees = thisMonthTransactions.reduce((sum, t) => sum + (Number(t.commission_fee) || 0), 0);
        ...
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="relative">
                    ...
                    <InputText v-model="searchQuery" placeholder="Search by reference number, description, or user name..." class="w-full pl-10" />
                </div>

                <!-- Filter Panel -->
                <div v-if="showFilters" class="grid grid-cols-1 gap-4 rounded-lg border border-gray-200 bg-gray-50 p-4 md:grid-cols-3 lg:grid-cols-4">
                    <!-- Type Filter -->
                    <div class="space-y-2">
                    ...
                    <div class="flex items-end">
                        <Button @click="clearFilters" outlined class="w-full border-gray-300 text-gray-600 hover:bg-gray-50"> Clear Filters </Button>
                    </div>
                </div>
            </div>

            <!-- Transactions List -->
            <Card class="flex-1 border border-gray-200 shadow-sm">
                <template #content>
                    ...
                    <div v-else class="space-y-4">
                        <div
                            ...
                            :key="transaction.key"
                            class="flex items-center justify-between rounded-lg border border-gray-200 bg-white p-4 transition-shadow duration-200 hover:shadow-md"
                        >
                            <!-- Left Side: Icon, Type & Description -->
                            <div class="flex items-center space-x-4">
                                <!-- Transaction Icon -->
                                <div :class="`rounded-full p-3 ${getTransactionColor(transaction.type)}`">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="mt-1 flex items-center gap-4">
                                        <span class="font-mono text-xs text-gray-500">{{ transaction.ref_number }}</span>
                            ...
                            <div class="hidden min-w-0 flex-1 flex-col items-center text-center lg:flex">
                                <div class="flex items-center gap-2 text-sm">
                                    <!-- From Account -->
                                    <div class="flex items-center gap-1 text-gray-600">
                                        <User class="h-3 w-3" />
                                        <span class="max-w-24 truncate">{{ transaction.sender_user_name }}</span>
                                    </div>

                                    <!-- Arrow -->
                                    <ArrowDownRight class="h-4 w-4 text-gray-400" />

                                    <!-- To Account -->
                                    <div class="flex items-center gap-1 text-gray-600">
                                        <User class="h-3 w-3" />
                                        <span class="max-w-24 truncate">{{ transaction.receiver_user_name }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <Button
                                    outlined
                                    size="small"
                                    class="border-gray-200 text-gray-400 hover:bg-gray-50"
            ...
            <Dialog v-model:visible="showDetailsDialog" modal header="Transaction Details" class="w-[600px]" @hide="closeTransactionDetails">
                <fieldset class="rounded-lg border border-gray-200 p-6">
                    ...
                    <div v-if="selectedTransaction" class="space-y-4">
                        <div class="flex items-center gap-3">
                            <div :class="`rounded-full p-3 ${getTransactionColor(selectedTransaction.type)}`">
                                <component :is="getTransactionIcon(selectedTransaction.type)" class="h-6 w-6" />
                            </div>
                            <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between text-sm">
                                ...
                                >
                                    {{ selectedTransaction.type === 'deposit' || selectedTransaction.type === 'topup' ? '+' : '-'
                            ...
                            <div v-if="selectedTransaction.commission_fee > 0" class="flex justify-between text-sm">
                                ...
                                <span class="text-orange-600">{{
                                    formatAmount(selectedTransaction.commission_fee, selectedTransaction.currency)
                                }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                        ...
                        <div v-if="selectedTransaction">
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div class="rounded-lg border bg-gray-50 p-4">
                                    <div class="mb-2 font-semibold text-gray-700">Debit</div>
                                ...
                                <!-- Credit Entry -->
                                <div class="rounded-lg border bg-gray-50 p-4">
                                    <div class="mb-2 font-semibold text-gray-700">Credit</div>
                                    <div class="mb-1 flex justify-between text-sm">
                                        <span class="text-gray-500">Account</span>
